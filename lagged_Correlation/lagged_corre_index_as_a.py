...
def main():
    ...
    def calculate_lagged_correlation(spy_path, vix_path, output_file, low_thres, high_thres, lookback, lookahead, time_diff):
        ...
        asset_a_change = asset_a_df[['Time', 'Index']]
    ...
    lookback = int(input("Enter the index window size in minutes: "))
    lookahead = int(input("Enter the lookahead period in minutes: "))

    low_thres = input("Enter range for negative index bin (Enter in format X,Y with no spaces): ")
    high_thres = input("Enter range for positive index bin (Enter in format X,Y with no spaces): ")
