...
def main():
    ...
    low_thresh = abs(float(input("Enter the threshold for negative movements: ")))
    high_thresh = abs(float(input("Enter the threshold for positive movements: ")))

    lookback = int(input("Enter the lookback period in minutes: "))
    lookahead = int(input("Enter the lookahead period in minutes: "))

    time_diff = int(input("Enter the difference in time between the lookback and lookahead windows (You probably want this at 0): "))

    calculate_lagged_correlation(file_a, file_b, output_file, low_thresh, high_thresh, lookback, lookahead, time_diff)
